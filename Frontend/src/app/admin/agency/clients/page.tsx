...
import { Plus, KeyRound, Ban, CheckCircle2, CreditCard } from 'lucide-react'
...
export default function AgencyClientsPage() {
  ...
  async function toggleActive(client: Client) {
    if (client.is_active) {
      ...
      const res = await fetch(`/api/agency/client-users/${client.id}`, { method: 'DELETE' })
      if (res.ok) load()
    } else {
      const res = await fetch(`/api/agency/client-users/${client.id}`, {
        method: 'PATCH',
        headers: { 'content-type': 'application/json' },
  ...
  async function resetPassword(client: Client) {
    ...
    const res = await fetch(`/api/agency/client-users/${client.id}/password`, {
      method: 'PATCH',
      headers: { 'content-type': 'application/json' },
  ...
  return (
    <AgencyShell>
      <div className="p-6 max-w-5xl">
        <PageHeader
          title="Clients"
          description="Create and manage client portal accounts from the agency CRM."
          action={
            <button

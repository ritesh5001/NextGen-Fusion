...
export const ecommerceServiceData: ServicePageData = {
  ...
  caseStudies: [
    { slug: "tatvivahtrends", name: "TatVivah Trends", niche: "Multi-vendor wedding ethnic wear", summary: "A three-thousand-product marketplace with occasion-based filtering, Razorpay checkout, digital gift cards and a ten-day returns and trust system — built so multiple vendors could sell into one storefront without the catalogue becoming unnavigable on a phone." },
    { slug: "clickngreet", name: "ClickNGreet", niche: "Gifting", summary: "A storefront built around product discovery and a checkout that holds up when demand concentrates into a handful of weeks a year. Occasion-driven traffic is where under-engineered stores fail, and where the engineering is worth paying for." },
    { slug: "newsaraswatisareecentre", name: "New Saraswati Saree Centre", niche: "Textiles", summary: "A traditional retailer moving online, where the real work was taxonomy: structuring a large seasonal range so category and product pages stop competing with each other for the same searches." },
  ],
  // Testimonials are omitted until we have real, attributable quotes. The
  // section does not render while this is empty.
  testimonials: [],
  deepDive: {
    heading: "What actually decides whether a store sells",
    body: [
      "Product photography and copy, first. A store with thin listings underperforms regardless of how it is built, and this is where nearly every launch we have seen slips — not in development. We ask for the catalogue in week one specifically so the delay surfaces early and visibly rather than late and expensively.",
      "Then checkout. Gateway choice in India comes down to settlement timing and which categories the provider will underwrite, not to the integration itself. Cash on delivery is where margin disappears quietly: without an RTO rule, a partial-prepaid nudge or a pincode restriction, returns cost more than the orders earn. Those get configured at launch, not after the first bad month.",
      "Then speed, measured on a mid-range Android over a throttled connection, because that is the condition your customers are actually in — not a desktop on office wifi. Core Web Vitals are part of the pre-launch checklist rather than a follow-up project.",
      "After the first hundred orders the work changes shape again. The catalogue grows and slows the site down, apps accumulate and start conflicting, sale configurations need building and unwinding, and every review widget or cart-recovery script adds weight that has to be paid for in performance somewhere. A store is an operating system for a business, not a project that ends at launch.",
    ],
  },
  pricing: {
  ...
  industries: {
    heading: "Categories we build for most",
    intro: "Most of our ecommerce work sits in categories with large, seasonal, photography-heavy catalogues — where merchandising and mobile performance decide revenue more than bespoke logic does.",
    items: [
      "Ethnic and wedding wear",
      "Sarees, textiles and handloom",
      "Jewellery and accessories",
      "Beauty and personal care",
      "Food, spices and speciality produce",
      "Gifting and occasion retail",
      "Home and lifestyle",
      "Agritech and rural supply",
      "Manufacturers selling direct",
    ],
  },
  locationLinks: [
    { slug: "ecommerce-development-company-in-lucknow", label: "Ecommerce development in Lucknow" },
    { slug: "website-development-company-in-mumbai", label: "Website development in Mumbai" },
  ],
  faqs: [
    { question: "Which platforms do you build e-commerce on?", answer: "Shopify where merchandising matters more than custom logic, WooCommerce where an existing WordPress estate makes migration more expensive than it is worth, and custom Next.js where the model is genuinely unusual. We tell you which one your project is, with reasoning, before you commit." },
...
export const webDesignServiceData: ServicePageData = {
  ...
  caseStudies: [
    { slug: "tatvivahtrends", name: "TatVivah Trends", niche: "Wedding ethnic wear marketplace", summary: "Occasion-based navigation — haldi, mehendi, sangeet — rather than a generic category tree, because that is how the buyer actually thinks. Design work that changed the information architecture, not just the surface." },
    { slug: "thegrafftee", name: "The Grafftee", niche: "HR tech", summary: "A firm with a strong offline reputation and no digital presence to match. The design problem was communicating the full breadth of services without burying the one a visitor came for, then converting to a demo booking." },
    { slug: "saurally", name: "Saurally Solar", niche: "Renewable energy retail", summary: "A considered-purchase category where the visitor is researching before buying. The design had to carry technical specification and trust signals at the same time without reading like a datasheet." },
  ],
  // Testimonials are omitted until we have real, attributable quotes.
  testimonials: [],
  deepDive: {
    heading: "How the design work actually runs",
    body: [
      "We design every template the site needs, not a homepage and a promise. The homepage is the easiest screen in any project and the least representative — the ones that decide whether the site works are the category page, the product or service detail page, the form, and the empty and error states nobody remembers to draw until a developer asks.",
      "Design starts from structure rather than from a moodboard. What has to be on this page, in what order, for someone who arrived from a search result and has not read anything else? Once that is settled the visual layer is comparatively quick. Doing it the other way round produces work that looks resolved and falls apart the moment real content goes in.",
      "You get a design system with it: a type scale, spacing rhythm, colour tokens and component states, so the site can be extended later without drifting into a dozen slightly different buttons. That is what makes the difference between a site that still looks deliberate after two years of edits and one that does not.",
      "Everything is checked on real devices rather than a resized browser window, because the layouts that break are almost always the ones between the breakpoints somebody designed for.",
    ],
  },
  pricing: {
  ...
  industries: {
    ...
    intro: "Mostly businesses where the site has a commercial job to do and somebody owns whether it does it — not brand exercises.",
    items: [
  ...
  locationLinks: [
    { slug: "website-development-company-in-lucknow", label: "Website development in Lucknow" },
    { slug: "website-development-company-in-mumbai", label: "Website development in Mumbai" },
  ],
  faqs: [
    { question: "Do you design without building?", answer: "Rarely, and we will tell you when it is a bad idea. Handing a design file to another team usually costs more in translation than it saves, because decisions that look free in Figma are expensive in code." },
...
export const seoServiceData: ServicePageData = {
  ...
  caseStudies: [
    { slug: "thegrafftee", name: "The Grafftee", niche: "HR tech", summary: "A recruitment firm invisible to the enterprise buyers searching for it. The fix was structural before it was editorial — a page per service that could rank, internally linked so the commercially important ones were not buried three clicks deep." },
    { slug: "krushidoctor", name: "Krushi Doctor", niche: "AgriTech retail", summary: "Narrow categories with genuine search intent behind them, which is exactly where a smaller business outranks a larger one. Most catalogues sit on more of these opportunities than their owners realise." },
    { slug: "newsaraswatisareecentre", name: "New Saraswati Saree Centre", niche: "Textiles", summary: "Taxonomy as SEO work: get the category structure wrong and every product page competes with its own category for the same query. Fixing that is usually worth more than any amount of new content." },
  ],
  // Testimonials are omitted until we have real, attributable quotes.
  testimonials: [],
  deepDive: {
    ...
    body: [
      "Weeks one to four are a technical pass and nothing else. Crawl and indexing errors, page speed, mobile layout faults, duplicate and thin pages, broken internal links, missing or incorrect structured data, and a canonical strategy that actually holds. It is unglamorous, it produces no content to show a board, and it is where the compounding starts.",
      "Weeks five to eight are architecture. Which pages should exist, which should be merged, which should be removed, and how they link to one another. Most sites we take on have their commercially important pages buried three clicks deep with nothing pointing at them, which is a way of telling Google those pages do not matter.",
      "Weeks nine to twelve are content and, for local businesses, the signals that decide the map pack: a Google Business Profile that is complete and maintained, and name, address and phone details that match exactly everywhere they appear. Link acquisition comes last, because pointing authority at a site that is not structurally sound wastes it.",
      "Throughout, the work happens in the codebase rather than in a document. That is the whole distinction between this and the retainers that run for a year and produce a folder of PDFs.",
    ],
  },
  pricing: {
  ...
  industries: {
    heading: "Where we do this most",
    intro: "Sectors where organic search is the main acquisition channel and the competition is winnable — rather than categories decided entirely by ad budget.",
    items: [
      "D2C and ecommerce",
  ...
  faqs: [
    ...
    { question: "Can you work on a site you did not build?", answer: "Yes, and most of our SEO clients are exactly that. We start with an audit so you can see what state it is in before committing to a retainer, and we work directly with whoever maintains it." },
  ],
